...
const baseURL =
    ...
    'https://eventhub.rahulshettyacademy.com'; // URL de la UI
const isCI = !!process.env.CI;
const screenshotMode = isCI ? 'only-on-failure' : 'on';
const videoMode = isCI ? 'retain-on-failure' : 'off';
const authStatePath =
    process.env.AUTH_STATE_PATH ?? path.join(__dirname, 'assets', 'auth', 'state.json');
...
const authenticatedProjects = projects.map((browser) => ({
    name: `auth-${browser.name}`,
    use: {
        ...browser.use,
...
const publicProjects = projects.map((browser) => ({
    name: `public-${browser.name}`,
    use: { ...browser.use },
